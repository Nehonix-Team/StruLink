...
def extract_advanced_features(input_data: Union[str, Dict]) -> Dict[str, float]:
    ...
    special_chars = len([c for c in url if not c.isalnum() and c not in ['/', '?', '=', '&', ':', '-', '.']])  # Exclude common URL chars
    ...
    features["suspicious_keyword_count"] = min(sum(1 for kw in SUSPICIOUS_KEYWORDS if kw.lower() in url.lower() and kw.lower() not in ['login', 'secure']), 2) / 2.0  # Exclude common benign keywords
    ...
    try:
        ...
        features["url_entropy_segments"] = calculate_segment_entropy(url)  # Entropy in different URL parts
        features["url_length_ratio"] = len(parsed.path) / max(len(url), 1)  # Proportion of URL that is path
        features["avg_param_length"] = calculate_avg_param_length(query_params)  # Average length of parameter values
        features["js_obfuscation_score"] = detect_js_obfuscation(url)  # Detect obfuscated JavaScript
        features["consecutive_special_chars"] = count_consecutive_specials(url)  # Count sequences of special chars
...
def create_ensemble_model():
    ...
    rf = RandomForestClassifier(
        n_estimators=50,  # Revert to previous value
        max_depth=8,     # Revert to previous value
        min_samples_split=50,  # Revert to previous value
        min_samples_leaf=20,   # Revert to previous value
        random_state=42
    )
    
    gb = GradientBoostingClassifier(
        n_estimators=50,  # Revert to previous value
        learning_rate=0.05,  # Revert to previous value
        max_depth=3,     # Revert to previous value
        random_state=42
    )
    ...
    xgb = XGBClassifier(
        ...
        learning_rate=0.05,  # Revert to previous value
        max_depth=3,     # Revert to previous value
        reg_lambda=2.0,  # Revert to previous value
        random_state=42
    )
    ...
    ensemble = VotingClassifier(
        ...
        voting='soft',
        weights=[1, 2, 2]  # Maintain weights
    )
